...
class WebSearchService:
    ...
    def search_github_repos(self, username: Optional[str] = None, topic: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # If topic is provided and valid, try to filter
...
class DocumentLoader:
    ...
    def load_pdf(path: str) -> str:
        ...
        try:
            ...
            logger.info(f"Loaded PDF: {path} - Length: {len(content)} chars")
            return content
        except Exception as e:
    ...
    def handle_tool_calls(self, tool_calls) -> tuple[List[Dict], List[str]]:
        ...
        for tool_call in tool_calls:
            ...
            if tool_name == "record_user_details":
                ...
                role_title = (arguments.get("role_title") or "").strip()

                if not job_desc or not role_title:
                    logger.info("Skipping evaluate_job_match: no job description or role title.")
                    result = {"error": "Job match evaluation requires both job description and role title"}
                else:
...
class CareerChatbot:
    ...
    def _create_system_prompt(self) -> str:
        """Create the system prompt using the chat_base.md template"""
        vars = {
            'config': self.config,
            'context': self.context
        }
    ...
    def chat(self, message: str, history: List[Dict[str, str]], max_retries: int = 3) -> str:
        """Main chat function that processes user messages with evaluation and retry approach"""
        ...
        # Evaluation with retry loop
        for attempt in range(max_retries):
            try:
                evaluation = self.evaluator.evaluate_structured(structured_reply, message, history)
